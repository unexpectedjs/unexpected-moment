(function (root, factory) {
    ...
    return {
        ...
        installInto: function (expect) {
            ...
            function diffMoments(actual, expected, output, diff, inspect, equal) {
                var actualIsUtc = actual.isUtc();
                var functionName = actualIsUtc ? 'moment.utc' : 'moment';
                ...
                output.jsFunctionName(functionName).text('(');
                if (momentShouldBe) {
                ...
                if (expectedIsDate) {
                    actualForDiff = actual.toDate().toString();
                    expectedForDiff = expected.toString();
                }
                if (expectedIsArray) {
                ...
                if (expectedIsObject) {
                    expectedForDiff = normalizeObject(expected);
                    actualForDiff = actual.toObject();
                    Object.keys(actualForDiff).forEach(function (key) {
                        if (!expectedForDiff.hasOwnProperty(key)) {
                            delete actualForDiff[key];
                        }
                    });
                }
                if (expectedIsString) {
                ...
                if (!subject.isSame(value)) {
                    return expect.fail({
                        diff: function (output, diff, inspect, equal) {
                            ...
                            return diffMoments(subject, value, output, diff, inspect, equal);
                        }
                    });
                }
            });
