    ...
    return {
        ...
        installInto: function (expect) {
            ...
            expect.addAssertion('<moment> to satisfy <moment|string|number|date|array|object>', function (expect, subject, value) {
                ...
                var valueIsDate = value instanceof Date;
                var valueIsObject = !valueIsDate && !valueIsArray &&
                                    valueType === 'object';
                var normalizedObjectValue = {};
                if (valueIsObject) {
                    ...
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        normalizedObjectValue[normalizedUnit] = value[unit];
                    }
                }
                        ...
                        diff: function (output, diff) {
                            if (valueIsDate) {
                                expect.argsOutput = function (output) {
                                    output.jsKeyword('new')
                                        .sp()
                                        .text('Date(')
                                        .appendInspected(value.toString())
                                        .text(')');
                                };
                            ...
                            if (valueIsObject) {
                                ...
                                Object.keys(subjectAsObject).forEach(function (key) {
                                    if (!(key in normalizedObjectValue)) {
