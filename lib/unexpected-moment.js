    ...
    installInto: function (expect) {
      ...
      function formatDiff(actual, expected, output, diff, inspect, equal) {
        ...
        if (expectedIsArray || expectedIsObject) {
          ...
          if (!isEqual) {
            output
              .append(inspect(actualForDiff))
              .sp()
              .annotationBlock(function () {
                this.error('should be ').append(inspect(expectedForDiff));
                if (formattedDiff) {
      ...
      expect.addAssertion(
        ...
        function (expect, subject, value, label) {
          // TODO: validate granularity?
          const granularity = (label || '')
            .replace(/^in /, '')
            .replace(/s$/, '');
      ...
      expect.addAssertion(
        '<moment> [not] to be [inclusively] between <moment|string|number|date|array|object> <moment|string|number|date|array|object>',
        function (expect, subject, from, to) {
          expect.argsOutput = function (output) {
          ...
          if (
            subject.isBetween(from, to, null, inclusivity) === expect.flags.not
          ) {
            return expect.fail();
          }
        }
      ...
      expect.addAssertion(
        ...
        function (expect, subject) {
          ...
          return expect(
            subject,
            '[not] to equal',
            subject.clone().startOf(unitOfTime)
          );
        }
      );

      expect.addAssertion(
        '<moment> [not] to be the end of (second|minute|hour|day|week|isoWeek|month|quarter|year)',
        function (expect, subject) {
          const unitOfTime = expect.alternations[0];
          return expect(
            subject,
            '[not] to equal',
            subject.clone().endOf(unitOfTime)
          );
        }
      );
    },
